...
\begin{absquote}
  ...
  |csvsimple(-l3)| provides a simple \LaTeX\ interface for processing files with
  comma-separated values (CSV).
  |csvsimple(-l3)| makes extensive use of key–value syntax, resulting in a straightforward interface.

  Filtering and table generation are particularly supported. As the package
  is intended to be lightweight, it does not provide support for data sorting or
  database storage.
\end{absquote}
...
An alternative to |csvsimple-l3| is the \refPkg{datatool} package
...
\refPkg{longtable}, \refPkg{tabularray}, \refPkg{booktabs}, \refPkg{ifthen}, and \refPkg{etoolbox}.
...
\begin{docCommands}[
  ...
  This macro reads the whole CSV file denoted by \meta{file name}
  with an automated formatting.
  For application, the package \refPkg{longtable} is required which has to be
...
\begin{docCommands}[
  ...
  This macro reads the whole CSV file denoted by \meta{file name}
  with an automated formatting.
  For application, the package \refPkg{booktabs} is required which has to be
  loaded in the preamble.
  The star variant treats the first line as data line and not as header line.
\begin{dispExample}
...
\begin{docCommands}[
  ...
  This macro reads the whole CSV file denoted by \meta{file name}
  with an automated formatting.
  For application, the packages \refPkg{booktabs} and \refPkg{longtable} are required which have to be
  loaded in the preamble.
  The star variant treats the first line as data line and not as header line.
\begin{dispListing}
...
\begin{docCommands}[
  ...
  with an automated formatting.
  For application, the package \refPkg{tabularray} is required which has to be
  loaded in the preamble.
  ...
  Options to the table environments from \refPkg{tabularray} may be given
...
is used as
mandatory argument (\refPkg{tabularray} inner specification).
Otherwise, \meta{taboptions 1} is used as
optional argument (\refPkg{tabularray} outer specification)
and
...
\begin{docCsvKey}{filter equal}{=\marg{stringA}\marg{stringB}}{no default}
  Only lines where \meta{stringA} and \meta{stringB} are equal after expansion
  are accepted.
  The implementation is done with the \refPkg{ifthen} package (loading required!).
\end{docCsvKey}
...
\end{dispListing}
  For |\iftest|, tests from the \refPkg{etoolbox} package like
...
\begin{docCsvKey}[][doc new=2016-07-01]{filter expr}{=\meta{boolean expression}}{no default}
  ...
  from the \refPkg{etoolbox} package is feasible (package loading required!).
  To preprocess the data line before testing the \meta{boolean expression},
  the option key \refKey{csvsim/before filter} can be used.
\begin{dispExample}
% \usepackage{etoolbox,booktabs}
...
\begin{docCsvKey}[][doc new=2016-07-01]{filter ifthen}{=\meta{boolean expression}}{no default}
  Only data lines which fulfill a \meta{boolean expression} are accepted.
  For the \meta{boolean expression}, every term from the \refPkg{ifthen} package
...
\begin{docCsvKey}{longtable}{=\meta{table format}}{style, no default}
  ...
  This requires the package \refPkg{longtable} (not loaded automatically).
\end{docCsvKey}
...
\begin{docCsvKey}[][doc new=2021-07-06]{tabularray}{=\meta{table format}}{style, no default}
  ...
  This requires the package \refPkg{tabularray} (not loaded automatically).
...
\begin{docCsvKey}[][doc new=2021-07-23]{long tabularray}{=\meta{table format}}{style, no default}
  ...
  from the package \refPkg{tabularray} (not loaded automatically).
\end{docCsvKey}
...
\begin{docCsvKey}[][doc new and updated={2021-09-09}{2023-12-18}]{generic collected table}{=\meta{name}}{no default, initially empty}
  ...
  collected content, e.g. |tblr| from package \refPkg{tabularray}, see \Fullref{sec:datacollection}.
...
the \refPkg{tabularray} package, collecting the data from the CSV file into a macro is needed.
...
\begin{docCommand}[doc new and updated={2021-07-06}{2023-12-17}]{csvexpval}{\meta{macro}}
  ...
  Alternatively, |\expandonce| from \refPkg{etoolbox} could be used.
\end{docCommand}
...
The \refPkg{siunitx} package provides a huge amount of formatting options for
numbers. A good and robust way to apply formatting by \refPkg{siunitx} inside
...
The \refPkg{tabularray} package gives extended control for generating
tables. \refKey{csvsim/tabularray} and \refKey{csvsim/centered tabularray}
support such tables. A distinctiveness is that for \refPkg{tabularray}
...
\item \refKey{csvsim/command} is \emph{collected} expanded. This is identical
  ...
  \item |\tablenum| from \refPkg{siunitx} is a robust command and therefore
...
Filters and line ranges can be used for \refPkg{tabularray} and all
...
table. Note that the \refPkg{tabularray} options are given as last
optional argument.
